...
DEFAULT_SETTINGS = {
    ...
    "price_markup_percent": 40, # dogesms ustiga % (sotuv narxi)
    "min_sell_price": 5000,     # eng past sotuv narxi (so'm)
    "auto_price_from_dogesms": True,  # sync da % bilan avto narx
    "only_available": True,     # faqat omborda borlari
    "bot_active": True,
    "maintenance": False,
...
def init_db():
    ...
    """)
    try:
        c.execute("ALTER TABLE country_prices ADD COLUMN IF NOT EXISTS available_count INTEGER DEFAULT 0")
    except Exception:
        try:
            c.execute("ALTER TABLE country_prices ADD COLUMN available_count INTEGER DEFAULT 0")
        except Exception:
            pass
    c.execute("""
...
def _fetch_one_telegram_price(code, name):
    ...
    try:
        ...
        for p in prices:
            ...
            if cents <= 0:
                continue
            avail = p.get("available_count")
            if avail is None:
                avail = p.get("count")
            if avail is None:
                avail = p.get("available")
            try:
                avail = int(avail) if avail is not None else 1  # noma'lum = bor deb hisob
            except Exception:
                avail = 1
...
def calc_sell_price_uzs(cents):
    """Dogesms cent → sotuv narxi (so'm), % ustiga + yuqoriga yaxlitlash"""
    rate = float(get_setting("usd_to_uzs", 12500) or 12500)
    markup = float(get_setting("price_markup_percent", 40) or 0)
    min_price = float(get_setting("min_sell_price", 5000) or 0)
    usd = cents / 100.0
    raw = usd * rate * (1 + markup / 100.0)
    price = int(math.ceil(raw))  # 23774.97 → 23775
    if min_price and price < min_price:
        price = int(min_price)
    return max(1, price)

def sync_dogesms_countries(progress_callback=None):
    """Mavjud eng arzon narxlarni yuklash. Omborda yo'q → yashirin."""
    ...
    count = 0
    seen = set()
    auto_price = get_setting("auto_price_from_dogesms", True)
    only_av = get_setting("only_available", True)
    for item in items:
        code = item["code"]
        seen.add(code)
        name = get_uz_name(code, item["name"])
        flag = get_flag(code)
        cents = int(item["price_cents"] or 0)
        avail = int(item.get("available") or 0)
        # available noma'lum bo'lsa (0) — baribir qo'shamiz, only_available filtri alohida
        is_active = 1
        if only_av and avail == 0 and item.get("available") is not None:
            # aniq 0 bo'lsa yashirish
            is_active = 0
        price_uzs = calc_sell_price_uzs(cents)
        c.execute("SELECT price, source FROM country_prices WHERE country_code = %s", (code,))
        row = c.fetchone()
        if row:
            if auto_price and row.get("source") in ("dogesms", "seed", None, ""):
                c.execute("""UPDATE country_prices SET dogesms_price_cents = %s, available_count = %s,
                             flag = %s, country_name = %s, price = %s, is_active = %s, source = 'dogesms'
                             WHERE country_code = %s""",
                          (cents, avail, flag, name, price_uzs, is_active, code))
            else:
                # qo'lda belgilangan narx saqlanadi
                c.execute("""UPDATE country_prices SET dogesms_price_cents = %s, available_count = %s,
                             flag = %s, country_name = %s, is_active = %s WHERE country_code = %s""",
                          (cents, avail, flag, name, is_active if only_av else 1, code))
        else:
            c.execute("""INSERT INTO country_prices
                         (country_code, country_name, price, dogesms_price_cents, available_count, flag, is_active, source)
                         VALUES (%s, %s, %s, %s, %s, %s, %s, 'dogesms')""",
                      (code, name, price_uzs, cents, avail, flag, is_active))
        count += 1
    # sync da kelmagan / tugaganlarni yashirish (faqat dogesms manbali)
    if only_av and seen:
        c.execute("""UPDATE country_prices SET is_active = 0, available_count = 0
                     WHERE source = 'dogesms' AND country_code <> ALL(%s)""", (list(seen),))
    conn.commit()
    put_db(conn)
    return count
...
def numbers_admin_menu():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    markup.add(types.KeyboardButton("🏷 Arzon mavjudlar"), types.KeyboardButton("🌍 Davlatlar ro'yxati"))
    markup.add(types.KeyboardButton("🌐 Barcha davlat kodlari"), types.KeyboardButton("📈 Foiz narx"))
...
def get_active_countries():
    """Faqat faol (+ ixtiyoriy mavjud) davlatlar, arzonidan"""
    conn = get_db()
    try:
        c = conn.cursor()
        only_av = get_setting("only_available", True)
        if only_av:
            c.execute("""SELECT country_code, country_name, price,
                                ...
                                COALESCE(available_count,0) AS available_count
                         FROM country_prices
                         WHERE is_active = 1
                           AND (COALESCE(available_count,0) > 0 OR source = 'manual')
                         ORDER BY price ASC""")
        else:
            c.execute("""SELECT country_code, country_name, price,
                                COALESCE(flag,'') AS flag,
...
def process_buy_country(call):
    ...
    bot.edit_message_text(
        ...
        f"⚠️ <b>Diqqat:</b>\n"
        f"• Nomer yaroqsiz / kod kelmasa — <b>pul balansga qaytariladi</b>\n"
        f"• Faqat <b>norasmiy Telegram</b> dan foydalaning\n"
        f"• Darhol <b>2FA (ikki bosqichli parol)</b> qoʻying\n"
        f"• Emailni oʻzgartiring\n"
        f"• Baʼzi raqamlar Telegramda bloklangan boʻlishi mumkin",
        call.message.chat.id, call.message.message_id, reply_markup=markup)
    bot.answer_callback_query(call.id)
...
def confirm_buy(call):
    ...
    if not order:
        ...
        bot.send_message(user_id,
            "❌ <b>Nomer olinmadi</b>\n\n"
            f"💰 <b>{format_money(price)}</b> balansingizga qaytarildi.\n"
            "Boshqa davlatni tanlab koʻring.",
            reply_markup=main_menu(user_id))
        return
    ...
    if not phone_number:
        ...
        put_db(conn)
        bot.send_message(user_id,
            "❌ <b>Nomer yaroqsiz yoki kelmadi</b>\n\n"
            f"💰 <b>{format_money(price)}</b> toʻliq balansingizga qaytarildi.\n"
            "Ishonchingiz uchun: kod/nomer boʻlmasa pul yechilmaydi.",
            reply_markup=main_menu(user_id))
        return
    ...
    # mavjudlikni kamaytirish
    try:
        c.execute("""UPDATE country_prices SET available_count = GREATEST(COALESCE(available_count,1)-1, 0)
                     WHERE country_code = %s""", (country_code,))
        c.execute("""UPDATE country_prices SET is_active = 0
                     WHERE country_code = %s AND COALESCE(available_count,0) <= 0 AND source = 'dogesms'""",
                  (country_code,))
    except Exception:
        pass
    conn.commit()
    ...
    code_line = f"\n🔐 SMS kod: <code>{sms_code}</code>" if sms_code else "\n🔐 SMS kod hali kelmadi — «Kodni yangilash» ni bosing."
    text = f"""✅ <b>Nomer berildi!</b>

📱 <code>{phone_number}</code>
{fl} {get_uz_name(country_code, country_name)}
💵 {format_money(price)}{code_line}

⚠️ <b>MUHIM OGOHLANTIRISH:</b>
1️⃣ Faqat <b>norasmiy Telegram</b> ilovasidan foydalaning
2️⃣ Darhol <b>2FA (ikki bosqichli parol)</b> yoqing
3️⃣ Email manzilni oʻzgartiring
4️⃣ Nomer ishlamasa / kod kelmasa — «Kodni yangilash»;
   tizimda nomer yaroqsiz boʻlsa pul qaytarilgan boʻladi"""
    markup = types.InlineKeyboardMarkup(row_width=1)
...
def numbers_admin(message):
    ...
    bot.send_message(message.chat.id,
                     ...
                     "🏷 Arzon mavjudlar — dogesms kod + aniq narx\n"
                     "📈 Foiz narx — % ustiga + avto yaxlitlash\n"
                     "🔄 yangilash — omborda yoʻqlar olib tashlanadi\n"
                     "🎁 Tekin — arzon mavjudlardan qoʻshing",
                     reply_markup=numbers_admin_menu())

@bot.message_handler(func=lambda m: m.text and "Arzon mavjud" in m.text)
def admin_cheap_available(message):
    if not is_admin(message.from_user.id):
        return
    conn = get_db()
    try:
        c = conn.cursor()
        c.execute("""SELECT country_code, country_name, price, dogesms_price_cents,
                            COALESCE(available_count,0) as available_count, COALESCE(flag,'') as flag
                     FROM country_prices
                     WHERE is_active = 1 AND COALESCE(dogesms_price_cents,0) > 0
                     ORDER BY dogesms_price_cents ASC, price ASC
                     LIMIT 60""")
        rows = c.fetchall()
    finally:
        put_db(conn)
    if not rows:
        bot.send_message(message.chat.id,
                         "❌ Roʻyxat boʻsh. Avval «🔄 dogesms dan yangilash» qiling.",
                         reply_markup=numbers_admin_menu())
        return
    # bo'lib yuborish
    chunk_size = 20
    for i in range(0, len(rows), chunk_size):
        ...
        text = f"🏷 <b>Arzon mavjud nomerlar</b> ({i+1}–{i+len(chunk)} / {len(rows)})\n"
        text += "Kod | Dogesms | Sotuv | Zaxira\n\n"
        for r in chunk:
            code = r["country_code"]
            fl = get_flag(code) if get_flag(code) != "🌍" else (r["flag"] or "🌍")
            name = get_uz_name(code, r["country_name"] or code)
            cents = r["dogesms_price_cents"] or 0
            usd = cents / 100
            text += (f"<code>{code}</code> {fl} {name}\n"
                     f"   🐕 ${usd:.2f} ({cents}¢) | 💵 {format_money(r['price'])} | 📦 {r['available_count']}\n\n")
        bot.send_message(message.chat.id, text[:4000])
    bot.send_message(message.chat.id,
                     "🎁 Tekin qoʻshish: kodni yuboring (masalan <code>CO</code>)\n"
                     "«🎁 Tekin nomer qoʻshish» tugmasi orqali.",
                     reply_markup=numbers_admin_menu())

@bot.message_handler(func=lambda m: m.text and "Foiz narx" in m.text)
def set_markup_percent(message):
    if not is_admin(message.from_user.id):
        return
    cur = get_setting("price_markup_percent", 40)
    min_p = get_setting("min_sell_price", 5000)
    msg = bot.send_message(
        message.chat.id,
        f"📈 <b>Foiz narx</b>\n\n"
        f"Hozir: <b>{cur}%</b> ustiga\n"
        f"Minimal sotuv: <b>{format_money(min_p)}</b>\n\n"
        f"Yangi foizni yozing (masalan <code>40</code>).\n"
        f"Keyin «🔄 dogesms dan yangilash» — barcha avto narxlar yangilanadi.\n"
        f"Qoʻlda belgilangan narxlar oʻzgarmaydi.\n/cancel",
        reply_markup=back_only())
    bot.register_next_step_handler(msg, process_markup_percent)

def process_markup_percent(message):
    if not is_admin(message.from_user.id):
        return
    if message.text in ["🔙 Orqaga", "/cancel"]:
    ...
    try:
        ...
        if val < 0 or val > 500:
            bot.send_message(message.chat.id, "0–500 oraligʻida yozing.", reply_markup=numbers_admin_menu())
            return
        set_setting("price_markup_percent", val)
        bot.send_message(message.chat.id,
                         f"✅ Foiz: <b>{val}%</b>\n"
                         f"Endi «🔄 dogesms dan yangilash» qiling — narxlar yangilanadi.",
                         reply_markup=numbers_admin_menu())
    except Exception:
        bot.send_message(message.chat.id, "Faqat son.", reply_markup=numbers_admin_menu())
