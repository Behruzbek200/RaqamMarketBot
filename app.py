...
def start_telethon_listener():
    ...
    Karta egasining Telegram akkaunti (TELETHON_PHONE) orqali ulanadi.
    """
    ...
    async def run_listener():
        ...
        print(f"🔌 Telethon ulanmoqda... session={session_path}")
        if phone:
            await client.start(phone=phone)
        else:
            await client.start()
        me = await client.get_me()
        print(f"✅ Telethon OK: id={me.id} username=@{me.username} phone={phone}")
...
📌 <b>Qoidalar:</b>
1. Aynan <b>{int(amount)}</b> soʻm tashlang (boshqa summa qabul qilinmaydi)
2. Toʻlov tushishi bilan balans <b>avtomatik</b> toʻldiriladi
3. Vaqt tugasa soʻrov bekor boʻladi — qayta «Hisob toʻldirish» qiling
4. Chek/sms kelmasa 1–3 daqiqa kuting"""
