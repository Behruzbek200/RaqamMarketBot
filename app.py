...
def my_ref_link(message):
    ...
    left = max(0, needed - (refs % needed) if needed else 0)
    text = f"""🔗 <b>Sizning referal havolangiz</b>

<code>{link}</code>

📋 <b>Qanday ishlaydi?</b>
1️⃣ Havolani doʻstingizga yuboring
2️⃣ Doʻst roʻyxatdan oʻtsa → sizga <b>{format_money(bonus)}</b>
3️⃣ Doʻst nomer sotib olsa → sizga <b>{percent}%</b>
4️⃣ Har <b>{needed}</b> ta haqiqiy doʻst → <b>1 tekin nomer</b>

📊 <b>Sizning natijangiz</b>
👥 Taklif qilganlar: <b>{refs}</b> ta
🎁 Tekin nomer huquqi: <b>{free}</b> ta
💸 Referal balansi: <b>{rbal}</b>
🎯 Keyingi tekin nomer uchun: yana <b>{left}</b> ta doʻst

⚠️ Faqat username bor (haqiqiy) akkauntlar hisobga olinadi."""
