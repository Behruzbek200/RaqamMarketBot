...
def get_active_countries():
    conn = get_db()
    c = conn.cursor()
    c.execute("""SELECT country_code, country_name, price, COALESCE(flag,''), COALESCE(dogesms_price_cents,0)
...
def build_countries_keyboard(page=0, per_page=10):
    ...
    for row in chunk:
        ...
        price = row["price"]
        flag = row["flag"] or get_flag(code)
...
def process_top_up_amount(message):
    ...
    expires = (now_dt + timedelta(minutes=int(minutes))).strftime("%Y-%m-%d %H:%M:%S")
    ...
    text = f"""➕ <b>Hisob toʻldirish</b>
{note}
💳 <b>Summa:</b> <code>{int(amount)}</code> soʻm
🏦 <b>Karta:</b> <code>{card}</code>
👤 <b>Egasi:</b> {owner}
⏳ Vaqt: <b>{minutes} daqiqa</b> (tugash: {expires})

⚠️ Aynan shu summani tashlang! Vaqt tugasa avtomatik bekor boʻladi."""
...
# Nomer sotib olish
@bot.message_handler(func=lambda m: m.text in ["📱 Nomer sotib olish", "🌍 Davlatlar ro'yxati"])
def select_country(message):
    if message.text == "🌍 Davlatlar ro'yxati" and is_admin(message.from_user.id):
        # Admin versiyasi pastroqda
        pass
    countries = get_active_countries()
    if not countries:
        bot.send_message(message.chat.id, "Davlatlar yoʻq. Admin: dogesms dan yangilang.",
                         reply_markup=main_menu(message.from_user.id))
        return
    markup, total, page = build_countries_keyboard(0)
    bot.send_message(message.chat.id, f"🌍 <b>Telegram nomerlari</b>\nJami: <b>{total}</b>\n\nDavlatni tanlang:",
                     reply_markup=markup)
...
if __name__ == "__main__":
    ...
    # Telethon Render da ishonchsiz — VPS da yoqing
